...
class EqualizerManager(private val audioSessionId: Int) {
    ...
    fun setEnabled(enabled: Boolean) {
        try {
            ...
            if (enabled) {
                // Restore band levels
                val currentLevels = _equalizerState.value.bandLevels
                for (i in currentLevels.indices) {
                    equalizer?.setBandLevel(i.toShort(), currentLevels[i].toShort())
                }
                // Restore bass boost
                if (_equalizerState.value.bassBoost > 0) {
                    bassBoost?.setStrength(_equalizerState.value.bassBoost.toShort())
                    bassBoost?.enabled = true
                } else {
                    bassBoost?.enabled = false
                }
                // Restore virtualizer
                if (_equalizerState.value.virtualizer > 0) {
                    virtualizer?.setStrength(_equalizerState.value.virtualizer.toShort())
                    virtualizer?.enabled = true
                } else {
                    virtualizer?.enabled = false
                }
                // Restore reverb
                val reverbPreset = _equalizerState.value.reverbPreset.toShort()
                ...
                // Turn off underlying hardware effects cleanly
                bassBoost?.enabled = false
                virtualizer?.enabled = false
                reverb?.enabled = false
            }
    ...
    fun setBandLevel(band: Int, level: Int) {
        try {
            if (_equalizerState.value.isEnabled) {
    ...
    fun setBassBoost(strength: Int) {
        try {
            ...
            val shouldEnable = _equalizerState.value.isEnabled && strength > 0
            bassBoost?.enabled = shouldEnable
            if (strength > 0) {
    ...
    fun setVirtualizer(strength: Int) {
        try {
            ...
            val shouldEnable = _equalizerState.value.isEnabled && strength > 0
            virtualizer?.enabled = shouldEnable
            if (strength > 0) {
    ...
    fun setReverbPreset(preset: Int) {
        try {
            ...
            val shouldEnable = _equalizerState.value.isEnabled && presetShort != PresetReverb.PRESET_NONE
